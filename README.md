...
Mini App을 설정한 봇에서는 `/start`의 `🚄 예약 화면 열기`, 채팅 입력창의
`예약 열기` 메뉴, 봇 프로필의 앱 버튼으로 **예약 전 과정을 한 앱 안에서** 할 수
있습니다 — 조건 입력, 실시간 열차 목록과 선택, 검색 시작과 시작 시각 예약,
진행 상황 확인과 중지, 결제 대기 예약 취소, 계정 등록, 즐겨찾기, 알림 설정.
`💬 채팅으로 예약`을 누르면 아래의 기존 단계형 흐름을 그대로 쓸 수 있고, 두
입구는 같은 세션과 같은 규칙을 공유합니다.

화면은 코레일·SR 로직을 갖지 않습니다. 봇의 API를 호출해 받은 결과를 그릴 뿐이며,
열차 조회도 예약 시작도 채팅이 쓰는 것과 같은 코드가 실행합니다. 그래서 봇이
페이지와 API를 같은 오리진으로 직접 서빙해야 하고, 공개 주소가 필요합니다 —
설정은 [DEPLOYMENT.md](DEPLOYMENT.md)의 *Telegram Mini App 배포*를 보십시오.
결제 정보는 어디서도 받지 않습니다.
...
|--------|------|------|
| `BOTTOKEN` | ✅ | 텔레그램 봇 토큰 |
| `MINI_APP_URL` | ❌ | 봇이 서빙하는 Mini App의 공개 HTTPS 주소. 미설정·비 HTTPS이면 기존 채팅 흐름만 사용 |
| `MINI_APP_API_ENABLED` | ❌ | Mini App API 리스너를 켭니다. `MINI_APP_URL`과 짝입니다 — 한쪽만 있으면 시작 시 경고 |
| `MINI_APP_API_PORT` | ❌ | 그 리스너의 포트. 기본 `8081`. **`FLASK_PORT`와 달라야 합니다** — 내부 콜백을 공개 소켓에서 떼어놓는 것이 목적입니다 |
| `MINI_APP_BIND_ADDRESS` | ❌ | 그 포트를 호스트에 매다는 주소. 기본 `127.0.0.1`. 인터넷 노출은 앞단 역프록시가 담당합니다 |
