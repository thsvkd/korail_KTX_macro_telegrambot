...
**`MINI_APP_URL`을 바꿀 때는 BotFather도 함께 고쳐야 합니다.** 채팅의 메뉴
버튼은 봇이 시작할 때마다 `setChatMenuButton`으로 스스로 맞추지만, 프로필의
**앱 열기**는 BotFather에만 저장되며 Bot API로는 읽을 수도 쓸 수도 없습니다
(`getMe`가 알려주는 것은 등록 여부인 `has_main_web_app`뿐입니다). 그래서 주소를
옮기면 두 입구가 조용히 서로 다른 페이지를 열게 됩니다 — 채팅은 새 주소로, 프로필은
예전 주소로. 고치려면 같은 메뉴에서 **Edit Mini App URL**을 고르십시오.
...
워크플로를 지우는 것만으로는 **이미 올라간 사이트가 내려가지 않습니다.** 새
배포가 멈출 뿐, 마지막 배포본은 계속 제공됩니다. 저장소 **Settings → Pages**에서
Source를 **None**으로 바꾸거나 `gh api -X DELETE repos/<소유자>/<저장소>/pages`
를 실행해 사이트 자체를 내리십시오.
