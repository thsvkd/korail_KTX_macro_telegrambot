...
def _matches(expected: Optional[str], provided: Optional[str]) -> bool:
    """
...
def verify_internal_request() -> bool:
    ...
    if request.remote_addr not in _LOOPBACK_ADDRESSES:
        logger.warning(
            f"Rejected internal callback from non-loopback address "
    ...
    if not _matches(settings.INTERNAL_CALLBACK_TOKEN, request.args.get('token')):
        logger.warning(
            f"Rejected internal callback with invalid token "
            f"from {request.remote_addr}"
        )
        return False
