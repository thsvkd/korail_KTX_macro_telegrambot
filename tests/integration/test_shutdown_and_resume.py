...
def _stop(service, child):
    ...
    with patch.object(service, "_owns_process", return_value=True), patch("os.kill", kill):
        service.shutdown()
    return signalled
...
class TestShutdownLeavesTheSearchRecoverable:
    ...
    def test_the_record_and_the_credentials_outlive_the_shutdown(self, storage, search_params):
        service = _service(storage)
        with patch.object(settings, "RUN_ID", FIRST_RUN):
            child = _start(service, search_params)
...
class TestRestartResumesTheSearch:
    ...
    def _restart(self, storage, search_params, resume_enabled=True):
        ...
        with (
            patch.object(settings, "RUN_ID", SECOND_RUN),
    ...
    def test_the_search_starts_again_with_the_same_parameters(self, storage, search_params):
    ...
    def test_the_credentials_survive_the_round_trip_through_redis(self, storage, search_params):
        """
    ...
    def test_the_user_is_told_the_search_resumed_rather_than_started(self, storage, search_params):
    ...
    def test_recovery_can_be_turned_off(self, storage, search_params):
        summary, _, popen = self._restart(storage, search_params, resume_enabled=False)
...
class TestAppSessionOutlivesTheRestart:
    ...
    def test_a_resumed_search_keeps_the_app_session_it_began_with(self, storage, search_params):
        first = _service(storage)
        with patch.object(settings, "RUN_ID", FIRST_RUN):
            child = _start(first, search_params)
