...
from korail_bot.release_notes import NOTES, notes_for
...
class TestWhenItSpeaks(AnnouncerFixture):
    ...
    def test_the_version_is_recorded_before_anyone_is_told(self):
        ...
        order = []
        self.storage.get_all_developers.return_value = [1]
        self.storage.set_announced_version.side_effect = lambda *a: order.append("record")
        self.telegram.send_to_multiple.side_effect = lambda *a: order.append("send") or 1
...
class TestWhatItSays(AnnouncerFixture):
    ...
    def test_a_release_with_notes_carries_them(self):
        announcer = ReleaseAnnouncer(self.storage, self.telegram, version=next(iter(NOTES)))

        assert notes_for(announcer.version) in announcer.message()
...
class TestTheReleaseNotes:
    ...
    def test_every_entry_says_something(self, version):
        assert NOTES[version].strip()
    ...
    def test_the_notes_are_written_for_the_user_not_the_author(self, version):
        ...
        for word in internals:
            assert word not in NOTES[version], f"v{version}: '{word}' 은 사용자의 말이 아닙니다"
