...
`make typecheck` 는 49개 소스 파일 전부에서 깨끗해야 하고, CI 의 Type check
단계도 실패하면 머지를 막습니다(2026-08-04부터).

오랫동안 그렇지 않았습니다. 11개 파일에 52건이 쌓여 있었고 CI 는
`continue-on-error: true` 로 그것을 넘겼는데, 실패해도 되는 단계의 출력은
아무도 읽지 않아서 나중에 들어온 진짜 결함이 같은 목록에 묻혔습니다. 그
안에는 타입 자리에 쓰인 내장 함수 `any`, None 검사 없는 속성 접근, 복호화
실패를 그대로 계정 레코드에 담는 코드가 있었습니다.

지금은 0건이므로 다음 1건이 곧 회귀입니다. 새 코드에는 타입을 붙여 주세요.
