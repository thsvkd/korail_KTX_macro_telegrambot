...
- **기본은 꺼짐입니다.** 원하지 않는 알림이 5분마다 오는 것은 침묵보다
  나쁩니다.
- `/notify` 로 간격을 고르거나, `/notify 10` 처럼 분 단위로 입력합니다.
  범위는 `PROGRESS_REPORT_MIN_MINUTES`~`PROGRESS_REPORT_MAX_MINUTES`
  (기본 1~180분). 끄려면 `/notify off`.
...
| `FAVOURITE_RENAME_TTL_SECONDS` | ❌ | 즐겨찾기 이름 입력 대기 시간 (기본 300) |
